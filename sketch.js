...
function draw() {
  ...
  // Glat bevægelse (smoothing)
  volSmooth = lerp(volSmooth, vol, 0.15);

  // Realistisk dB beregning
  ...
  const leftCY = topH * 0.53;
  const R = min(width / 2, topH) * 0.56;
  ...
  // Hint hvis ikke aktiv
  if (!aktiv) {
    fill(0, 80);
    rect(0, topH, width / 2, bottomH);
    fill(255);
    textSize(bottomH * 0.28);
    text('Tryk her\nfor at starte', width * 0.25, topH + bottomH / 2);
  }
}
